...
test: build
	...
	$(BIN) --reverse 100 | grep -qF 'no expression found' || { echo 'FAIL: --reverse 100 should find nothing'; exit 1; }; \
	if $(BIN) --reverse abc >/dev/null 2>&1; then echo 'FAIL: invalid target accepted'; exit 1; fi; \
